...
Page({
  ...
  loadTodos() {
    ...
    // 如果是周末，过滤掉类型为1的事项
    const filteredList = isWeekend ? updatedList.filter(item => item.category !== '0') : updatedList;

    console.log('所有事项数据：', filteredList) // 打印所有事项数据
    // 分组并设置数据
    const groupedList = this.groupByCategory(filteredList);
